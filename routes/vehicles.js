...
// ملف مستقل تماماً — لا يؤثر على باقي التطبيق
// ============================================================
...
module.exports = function registerVehicleRoutes(app, deps) {
    ...
    const missing = Object.keys(requiredFns).filter(k => typeof requiredFns[k] !== 'function');
    if (missing.length > 0) {
        console.error('❌ [VEHICLES] Missing dependencies:', missing.join(', '));
        return;
    }
    ...
    // ============================================================
    // 🎨 FORMATTER — تنسيق بيانات الوسيلة
    // ============================================================
    ...
    app.get('/api/vehicles', authenticateAccessToken, requirePermission('vessels:read'), async (req, res) => {
        try {
            ...
            const formatted = vehicles.map(formatVehicle);
            ...
            res.status(500).json({
                success: false,
                error: 'فشل تحميل الوسائل البرية'
            });
        }
    });

    // ============================================================
    // ✅ POST — إضافة وسيلة برية جديدة
    // ============================================================
    app.post('/api/vehicles',
        ...
        async (req, res) => {
            try {
                ...
                // ✅ التحقق من المنطقة (إلزامي)
                if (typeof region !== 'string' || !region.trim()) {
                    return res.status(400).json({
                        success: false,
                        error: 'المنطقة / الإقليم مطلوب'
                    });
                }

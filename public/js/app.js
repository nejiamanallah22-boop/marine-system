...
async function showPage(page) {
    const pages = ['pageMain', 'pageMaint', 'pageEff', 'pageSupport', 'pageTrack', 'pageMap', 'pageUsers', 'pageNote'];
    pages.forEach(p => {
    ...
    if(page === 'main') { 
        ...
        const el = document.getElementById('pageMaint');
        if (el) el.classList.remove('hidden'); 
        await renderMaint();
    }
    else if(page === 'eff') { 
        const el = document.getElementById('pageEff');
        if (el) el.classList.remove('hidden'); 
        await renderEff();
    }
    else if(page === 'support') { 
        const el = document.getElementById('pageSupport');
        if (el) el.classList.remove('hidden'); 
        await renderTickets();
    }
    else if(page === 'track') { 
        if(isAdmin) { 
            const el = document.getElementById('pageTrack');
            if (el) el.classList.remove('hidden'); 
            await loadTrackUsers();
            startTrackUsers();
        } else { 
        ...
        if(isAdmin) {
            const el = document.getElementById('pageMap');
            if (el) el.classList.remove('hidden');
            setTimeout(() => {
        ...
        if(isAdmin) { 
            const el = document.getElementById('pageUsers');
            if (el) el.classList.remove('hidden'); 
            await renderUsers();
        } else { 
            showToast("غير مسموح - هذه الصفحة للمسؤول فقط", true); 

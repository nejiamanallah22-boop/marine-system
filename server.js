...
const JWT_SECRET = process.env.JWT_SECRET || 'my_super_secret_key_change_me_in_production';
...
// ================= مسارات التذاكر (مع الرد والإغلاق) =================
...
app.post('/api/users', auth, admin, async (req, res) => {
    ...
    if (exists) return res.status(400).json({ error: 'اسم موجود' });
...
app.put('/api/users/:id', auth, admin, async (req, res) => {
    ...
    if (req.body.name && req.body.name !== user.name) {
        ...
        if (dup) return res.status(400).json({ error: 'اسم موجود' });
...
async function initDatabase() {
    ...
    if (!adminExists) {
        ...
        console.log('✅ المستخدمون الافتراضيون تم إنشاؤهم');
    }
    const countVessels = await Vessel.countDocuments();
    if (countVessels === 0) {
        ...
        console.log('✅ مراكب افتراضية تم إنشاؤها');
    }
}
